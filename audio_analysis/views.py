...
import os
from . import utils
from .performance import EvaluatePerformanceClass
...
class CompareTwoAudiosEndpoint(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not utils.calculate_similarity(
                utils.predict_instrument(query_audio_full_path),
                utils.predict_instrument(ref_audio_full_path))["is_similar"]:
                return Response({
                    "success": False,
                    "ai_response": {
                        "score":0,
                        "recommendation": "The instrument detected does not match with the reference track. Please try again"
                    }
                }, 
                status=200)
